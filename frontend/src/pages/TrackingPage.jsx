...
// Modal is only shown when cancelling an ACCEPTED request (reason required + helper notified)
...
const css = `
  ...
  /* Cancel — open request: red, direct, no modal */
  .tp-cancel-open {
    ...
    text-transform:uppercase; letter-spacing:0.1em; font-family:'Outfit',sans-serif;
    transition:background 0.15s,box-shadow 0.15s,transform 0.12s; cursor:pointer;
  }
  .tp-cancel-open:hover:not(:disabled) {
    background:rgba(220,38,38,0.13)!important;
    box-shadow:0 4px 16px rgba(220,38,38,0.18)!important;
    transform:translateY(-1px);
  }
  .tp-cancel-open:active:not(:disabled) { transform:translateY(0); }
  .tp-cancel-open:disabled { opacity:0.5; cursor:not-allowed; }

  /* Cancel — accepted request: amber, opens modal for reason */
  .tp-cancel-accepted {
    display:inline-flex; align-items:center; justify-content:center; gap:0.5rem;
    padding:10px 22px; border-radius:999px;
    background:rgba(245,158,11,0.08); border:1.5px solid rgba(245,158,11,0.4);
    color:#b45309; font-size:0.72rem; font-weight:700;
    text-transform:uppercase; letter-spacing:0.1em; font-family:'Outfit',sans-serif;
    transition:background 0.15s,box-shadow 0.15s,transform 0.12s; cursor:pointer;
  }
  .tp-cancel-accepted:hover {
    background:rgba(245,158,11,0.15)!important;
    box-shadow:0 4px 16px rgba(245,158,11,0.2)!important;
    transform:translateY(-1px);
  }
  .tp-cancel-accepted:active { transform:translateY(0); }

  /* Disabled pill — completed or already cancelled */
  .tp-cancel-disabled {
    display:inline-flex; align-items:center; justify-content:center; gap:0.5rem;
    padding:10px 22px; border-radius:999px;
...
// ─────────────────────────────────────────────────────────────────────────────
// TrackingPage — registered users only
// Guests are handled exclusively by GuestTracking.jsx
// ─────────────────────────────────────────────────────────────────────────────
const TrackingPage = () => {
  ...
  const [showCancelModal, setShowCancelModal] = useState(false); // only for accepted
  const [cancelLoading,   setCancelLoading]   = useState(false);

  const pollRef = useRef(null);

  // ── Guard: redirect guests away ───────────────────────────────────────────
  useEffect(() => {
  ...
  const fetchRequest = useCallback(async () => {
    try {
      ...
      setRequest(res.data);
      setError(null);
      // Sync cancelled state from server (covers page-reload after cancel)
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // handleCancelOpen — OPEN request
  // No modal, no reason. Fires immediately on button click.
  // Backend stores "No reason provided" automatically.
  // ─────────────────────────────────────────────────────────────────────────
  const handleCancelOpen = useCallback(async () => {
    ...
    try {
      ...
      toast.error(
        err.response?.data?.message || 'Could not cancel request. Please try again.'
      );
    } finally {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // handleCancelAccepted — ACCEPTED request
  // Called by CancelRequestModal once the user picks a reason.
  // The reason is persisted and the socket event notifies the helper.
  // ─────────────────────────────────────────────────────────────────────────
  const handleCancelAccepted = useCallback(async (reason) => {
    setCancelLoading(true);
    try {
      ...
      clearInterval(pollRef.current);
      await fetchRequest();
      toast.success('Request cancelled. The helper has been notified.');
    } catch (err) {
      toast.error(
        err.response?.data?.message || 'Could not cancel request. Please try again.'
      );
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center"
      ...
      <div className="text-center">
        ...
        <p style={{ fontSize:'0.72rem',color:'#9ca3af',fontWeight:700,textTransform:'uppercase',letterSpacing:'0.16em' }}>
          Loading request…
        </p>
      </div>
    </div>
  ...
  const statusConfig = cancelled
    ? { icon:'🚫', color:'#dc2626', bg:'rgba(254,242,242,0.85)', border:'rgba(220,38,38,0.2)',
        label:'Cancelled',
        desc:'Your request has been cancelled. No helpers will be alerted.' }
    : isCompleted
    ? { icon:'✅', color:'#15803d', bg:'rgba(240,253,244,0.85)', border:'rgba(74,222,128,0.35)',
        label:'Completed',
        desc:'This request has been completed. Thank you for using HelpLink!' }
    : isAccepted
    ? { icon:isSOS?'🚨':'🙌', color:'#15803d', bg:'rgba(240,253,244,0.85)', border:'rgba(74,222,128,0.35)',
        label:'Helper Assigned',
        desc: isSOS
          ? 'HELP IS ON THE WAY. Stay where you are and keep your phone available.'
          : 'A helper has accepted your request and is on the way.' }
    : { icon:isSOS?'🆘':'🔍', color:'#dc2626', bg:'rgba(254,242,242,0.85)', border:'rgba(220,38,38,0.2)',
        label:isSOS?'SOS Active':'Searching…',
        desc: isSOS
          ? 'HELP IS ON THE WAY — alerting nearby responders.'
          : 'Your request has been posted. Searching for nearby helpers.' };

  // ─────────────────────────────────────────────────────────────────────────
  // renderCancelControl
  //
  // OPEN      → red button → direct cancel, no modal, no reason asked
  // ACCEPTED  → amber button → opens CancelRequestModal, reason required,
  //             backend persists reason + socket notifies helper
  // COMPLETED → grey disabled pill (cannot cancel)
  // CANCELLED → grey disabled pill (already done)
  // ─────────────────────────────────────────────────────────────────────────
  const renderCancelControl = () => {
    ...
    if (isAccepted) {
      // Helper already on the way — require reason so helper is notified why
      return (
        <button
          type="button"
          className="tp-cancel-accepted"
    ...
    // isPending — open, cancel immediately, no reason needed
    return (
      <button
        type="button"
        className="tp-cancel-open"
  ...
  return (
    <>
      {/*
        CancelRequestModal is shown only for ACCEPTED cancellations.
        For OPEN requests the cancel fires directly via handleCancelOpen —
        no modal is shown, no reason is collected.
      */}
      <CancelRequestModal
        ...
        <div className="relative z-10 w-full max-w-sm sm:max-w-md mx-auto px-4 sm:px-6">
          ...
          {isSOS && !isCompleted && !cancelled && (
            <div className="tp-card mb-4 sm:mb-5 flex items-center gap-3 rounded-2xl px-5 py-4"
              ...
              <div>
                <div style={{ fontSize:'0.6rem',fontWeight:700,textTransform:'uppercase',letterSpacing:'0.18em',color:'rgba(255,255,255,0.65)',marginBottom:2 }}>
                  SOS Mode
                </div>
          ...
          <div className="tp-card-1 gt-card-shadow rounded-3xl overflow-hidden mb-4"
            ...
            <div className="px-6 sm:px-9 pt-8 sm:pt-10 pb-6 sm:pb-8">
              ...
              <div className="flex justify-center mb-7 sm:mb-8">
                <div className="relative" style={{ width:90,height:90 }}>
                  ...
                  <div style={{
                    ...
                    background:statusConfig.bg, border:`1.5px solid ${statusConfig.border}`,
              ...
              <div className="flex justify-center mb-5 sm:mb-6">
                <div style={{
                  ...
                  <span style={{
                    ...
                    animation: !isAccepted && !isCompleted && !cancelled
                      ? 'dotPulse 1.2s ease-in-out infinite' : 'none',
                  }}/>
                  {statusConfig.label}
                </div>
              </div>
              ...
              {/* Enhance Request — only when pending */}
              {user && isPending && (
                <div className="flex justify-center mb-4" style={{ animation:'fadeUp 0.4s 0.15s ease both' }}>
                  <button
              ...
              {/* ── Cancel control — all non-completed states ── */}
              {user && (
              ...
              {/* Cancellation reason card — rendered after cancellation */}
              ...
              {/* Guidance tips — hidden after cancel or complete */}
              {guidance.length > 0 && !cancelled && !isCompleted && (
                <div className="mt-5 px-5 py-4 rounded-2xl"
                  ...
                  <div className="text-[0.65rem] font-bold uppercase tracking-[0.18em] text-red-500 mb-3">
                    🚨 Immediate Guidance
                  </div>
              ...
              {/* Helper card — hidden after cancellation */}
              {hasHelper && !cancelled && (
                <div className="mb-4 rounded-2xl p-4 sm:p-5"
                  ...
                  <p style={{ fontSize:'0.56rem',fontWeight:800,textTransform:'uppercase',letterSpacing:'0.2em',color:'#15803d',marginBottom:'0.75rem' }}>
                    Assigned Helper
                  </p>
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p style={{ fontWeight:600,color:'#111827',fontSize:'0.92rem',margin:0 }}>
                        {request.helper?.name || 'Helper'}
                      </p>
              ...
              {/* Map preview — hidden after cancellation */}
              {canShowMap && !cancelled && (
                <button
                  onClick={() => navigate(`/map/${request._id}`)}
                ...
                >
                  ...
                  <div className="flex items-center justify-between px-4 py-3">
                    ...
                    <div className="flex items-center gap-1.5 rounded-full px-3 py-1" style={{ background:'#dc2626' }}>
                      <span style={{ fontSize:'0.6rem',fontWeight:800,textTransform:'uppercase',letterSpacing:'0.12em',color:'#fff' }}>
                        Open Map
                      </span>
              ...
              {hasHelper && !canShowMap && !isCompleted && !cancelled && (
                <div className="rounded-xl px-4 py-3 mb-3 text-center"
                  ...
                  <p style={{ fontSize:'0.68rem',color:'#9ca3af',margin:0 }}>
                    📍 Map tracking will be available once location data is ready
                  </p>
                </div>
              )}
            ...
            <div className="px-6 sm:px-9 py-3.5 flex items-center justify-center"
              ...
              <div style={{
                ...
                <a href="tel:112" style={{ fontWeight:800,color:'#dc2626',textDecoration:'none' }}>
